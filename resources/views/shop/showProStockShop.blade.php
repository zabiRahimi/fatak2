...
@section('dash_content')
  ...
  @else
    ...
    <div class="div_search_c">
      <button type="button" class="btn btn_search_c" onclick="pro_searchC('proCheckSPSS', 'proSPSS', 'all' , '' , '' , 'showProStockShop' , '' )">همه محصولات</button>
      <div class="div_search_form_date_c" action="index.html" method="post">
        <input type="text" class="input_date_c input_pro_date_c placeholder" id="pro_searchSPSS" placeholder="نام محصول">
        <button type="button" class="btn_date_c btn" onclick="pro_searchC('proCheckSPSS', 'proSPSS', 'pro' , 'pro_searchSPSS' , '' , 'showProStockShop' , 1 )"><i class="fas fa-search"></i></button>
      </div>
      <div class="div_search_form_date_c" action="index.html" method="post">
        <input type="text" class="input_date_c input_code_date_c placeholder"  id="id_searchSPSS" placeholder="کد محصول">
        <button type="button" class="btn_date_c btn"onclick="pro_searchC('proCheckSPSS', 'proSPSS', 'id' , '' , 'id_searchSPSS' , 'showProStockShop' , 2 )"><i class="fas fa-search"></i></button>
      </div>
      </div>
    <div class="dashLBodySh">
      ...
      @if (empty($pro[0]['id']) and $notRecord=='ok')
        <div class="div_alert alert alert-danger">
          تا کنون هیچ محصول ثابتی ثبت نشده است .
        </div>
      ...
      @foreach ($pro as $value)
        ...
        ?>
        <a href="/showProOneStockShop/{{$value->id}}" class="showProUSSA">
