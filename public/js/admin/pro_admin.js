...
function orderBackSabt() {
  buy_id =$('#code_ersalOrder').val(),
  $.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')}});
  $.ajax({
    type:'get',
    url:'../../orderBackSabt/' + buy_id ,
    success:function(sd){
      window.location.href  = "/orderBackSabt/" + buy_id;
    },
    error : function(xhr){
      var errors = xhr.responseJSON;var error=errors.errors;
      if(error['buy_id']) {
...
function orderBackSave(buy_id,pro_id,shop_id) {
  $.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')}});
  $.ajax({
    type:'post',
    url:'../../orderBackSave' ,
    ...
    success:function(sd){
      ...
      $('#orderModalPro').modal('show');
      $("#orderModalPro").on('hide.bs.modal', function () {
      window.location.href  = "/orderBackSabt";
      });
    },
    error : function(xhr){
      var errors = xhr.responseJSON;var error=errors.errors;
...
function orderBackEdit(backPro_id) {
  $.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')}});
  $.ajax({
    type:'post',
    url:'../../orderBackEdit' ,
